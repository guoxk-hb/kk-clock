<template>
  <div class="dragable 
  box-border w-full min-w-full
 ">
    <div class="box-border px-[8vw] py-[4vw] flex justify-between
    border-soild rounded-md
    ...
    ">
      <template v-for="(item, index) in Object.values(time)" :key="index">
        <div class="box-border flex-3 text-center" v-if="index > 3">
          <span class="align-text-top text-shadow text-[20vw] text-white font-sans ">{{ item }}</span>
        </div>
    ...
    box-border
    my-2 px-3 py-[1vw]
    border-soild rounded-md border-1
  border-gray-300 bg-violet-200 
    shadow-lg 
    ">
      <div class="overflow-hidden">
        <span class="whitespace-nowrap text-[5vw]">代办事项:周六晚7点要塞 7点要塞 7点要塞 7点要塞 7点要塞 </span>
      </div>
    </div>
  </div>
</template>
