<template>
	<div class="dragable box-border w-full min-w-full">
		<div
			class="box-border px-[8vw] py-[4vw] flex justify-between border-soild rounded-md border-1 border-gray-300 bg-violet-200 shadow-lg"
		>
		...
		>
			<div ref="taskText" class="overflow-x-scroll overflow-y-hidden">
				<span class="whitespace-nowrap text-[5vw]"
					>代办事项:周六晚7点要塞 7点要塞 7点要塞 7点要塞 7点要塞
				</span>
				<span class="whitespace-nowrap text-[5vw]"
					>代办事项:周六晚7点要塞 7点要塞 7点要塞 7点要塞 7点要塞
				</span>
		...
		<div
			class="undragable flex justify-around p-[2vw] rounded-md hover:bg-slate-100/20 hover:opacity-100 opacity-0"
		>
...
<script setup lang="ts">
	...
	const taskText: HTMLDivElement | null = ref(null)
	...
	onMounted(() => {
		...
		console.log(
			taskText?.value,
			taskText?.value.scrollWidth,
			taskText?.value.clientWidth,
			taskText?.value.scrollLeft
		)
	})
</script>
